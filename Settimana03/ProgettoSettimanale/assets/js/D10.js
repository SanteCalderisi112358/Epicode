...
*/
{
  ...
  function deleteOne(string, boolean) {
    if (boolean === true) {
      ...
      return string.slice((-string.length - 1), -1);
    }
  }
  console.log(deleteOne(mioArray, false));
...
*/
{
  console.log('Esercizio 5: DA RIVEDERE!!');

  const onlyLetters = (string) => {
    let stringSplit = string.split('');
    for (let i = 0; i < stringSplit.length; i++) {
      if (typeof (parseInt(stringSplit[i])) === 'number') {
        delete stringSplit[i];
      }
    }
...
*/
{
  console.log('Esercizio 8: DA RIVEDERE');
  const object = {};
}
/* ESERCIZIO 9
...
*/
{
  ...
  console.log(isTodayMyBirthday(14, 04));
}
// Arrays & Oggetti
...
*/
{
  console.log('Esercizio 11: DA RIVEDERE')
}
/* ESERCIZIO 12
  ...
  const newestMovie = (array) => {
    ...
    for (i = 0; i < array.length; i++) {
        year.push(parseInt(array[i].Year));
         ...
         index = year.indexOf(max);
        
         newMovie = array[index].Title;
      

    }
    return `Il film più recente è ${newMovie}, uscito nel ${max}`;
  }

  // return `Il film più recente è ${result}`;
...
*/
{
  ...
  const onlyInLastMillennium = (array) => {
    ...
    let moviesMillennials = [];
    for (i = 0; i < array.length; i++) {
      if (array[i].Year < 2000) {

        console.log(`${movies[i].Title}. Anno di uscita: ${movies[i].Year}`);
      }
    }
...
*/
{
  console.log('Esercizio 18: DA RIVEDERE');
  ...
  const searchAndDivide = (string) => {
    ...
    for(i=0; i < movies.length; i++) {
      ...
      if (movies[i].Title.search(string) > -1) {
        ...
        object.match += movies[i].Title;

      }else{
        object.unmatch +=movies[i].Title;
      }
